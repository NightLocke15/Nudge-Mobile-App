...
function DiaryLogs() {
    ...
    function addLog() {
        ...
        if (user.idnum === localUser) {
            return {
                ...user,
                logs: [...user.logs, {id: user.logs.length, name: `Untitled Note ${user.logs.length}`, type: "Diary", text: "blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah "}]
            }
        }
        else {
            return user;
    ...
    const logRendered = ({ item }) => {
        return (
            <View key={item.id} style={[stylesLight.logContainer, {width: width/2 - 20, marginLeft: 5, marginRight: 5 }]}>
                <Pressable onLongPress={() => triggerEditing(item)}>
                    <Text style={stylesLight.logName}>{item.name}</Text>
                    <Text numberOfLines={7} ellipsizeMode="tail" style={stylesLight.logText}>{item.text}</Text>
                </Pressable>
            </View>
        )
    }
    ...
    return (
        <SafeAreaView style={stylesLight.container}>
            <LinearGradient style={stylesLight.contentContainer} colors={["#ffffff", "#aaaaaa"]}>
                ...
                <SwipeListView data={localUserInfo[0] && localUserInfo[0].logs}
